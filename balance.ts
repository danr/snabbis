...
const show = (s: any) => console.log(JSON.stringify(s, undefined, 2))
...
function removeNoise(entry: Entry): Entry {
...
const buf = fs.readFileSync('snabbis.d.ts')
const s = buf.toString()
const m = balance(s, [["/**", "*/"], ["{", "}"]], [';'])

const doc = documentation(traverse(headers, m))
const md =
  (e => linearise(e, 3))(
  mapEntries(explicitHierarchy,
  mapEntries(removeNoise,
  filterEntries(e => e.header.search(/(private|import)/) == -1,
  doc))))
...
function doctest(e: Entry): Entry {
  let name = 'unknown'
  const m = e.header.match(/\w+(?=[(<:])/)
  if (m != null) {
    name = m[0].trim()
  }
  let has_tests = false
  //show({header: e.header, name})
  if (e.docstring.length == 0) {
    return e
  }
  const gs = balance(e.docstring, [['```', '```']])
  //show({ds: e.docstring})
  //show({gs: gs})
  let pre = ''
  gs.map(g => {
    if (g.kind == 'block') {
      /**
      inner(g) can look like one of these:

      "typescript
      import { tag, * as S } from 'snabbis'"
      ...
      '<div>'"
      */
      const s = inner(g).split('\n').slice(1).map(s => s.trim())
      if (s[0][0] == '>') {
        let expression = ''
        while (s.length > 0) {
          if (s[0][0] == '>') {
            expression += s[0].slice(1)
            s.shift()
          } else {
            break;
          }
        ...
        has_tests = true
        const value = s.join('\n')
        console.log('test("' + name + ' ' + ++tests + '", assert => {')
        ...
        // preamble
        if (!has_tests) {
          console.log(s[0].replace(/'snabbis'/g, "'../src/snabbis'"))
        } else {
          pre += s.join('\n')
        }
        //show({pre, s})
...
//const pushed = mapEntries(push_docstrings_down, doc)
console.log('import * as test from "tape"')
mapEntries(doctest, doc)
